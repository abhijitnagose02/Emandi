...
export default function LiveNegotiationChat() {
  ...
  if (!negotiation) {
    return (
      <div className="p-8 text-center bg-white rounded-2xl border border-emerald-200">
        <AlertCircle size={32} className="text-emerald-300 mx-auto mb-3" />
        <h3 className="font-bold text-emerald-900 text-base">No Active Negotiations</h3>
  ...
  return (
    <div className="bg-white rounded-3xl border border-emerald-200 overflow-hidden shadow-sm flex flex-col min-h-[640px] max-h-[85vh]">
      
      {/* 1. Turn & Role Simulation Banner */}
      <div className="bg-emerald-50 text-emerald-900 px-4 py-2.5 flex flex-wrap items-center justify-between gap-3 text-xs border-b border-emerald-200">
        <div className="flex items-center gap-2">
          <span className="text-emerald-700 font-semibold">Viewing Perspective:</span>
          <span className="font-bold flex items-center gap-1.5 bg-white px-2.5 py-1 rounded-lg border border-emerald-300 shadow-sm">
            {isFarmer ? <Tractor size={14} className="text-emerald-600" /> : <Users size={14} className="text-emerald-600" />}
        ...
        <div className="flex items-center gap-2">
          {!isAccepted && !isRejected && (
            <>
              {isMyTurn ? (
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full font-extrabold bg-emerald-200 text-emerald-900 animate-pulse text-[11px] shadow-sm">
                  <span className="w-2 h-2 rounded-full bg-emerald-900"></span>
                  YOUR TURN TO DECIDE
                </span>
              ) : (
                <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full font-bold bg-emerald-500/20 text-emerald-600 border border-emerald-500/30 text-[11px]">
      ...
      <div className="p-4 bg-emerald-50/80 border-b border-emerald-200 flex flex-wrap items-center justify-between gap-4 z-10">
        <div>
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-white border border-emerald-200 px-2.5 py-0.5 rounded-full">
          ...
          <h3 className="font-black text-emerald-900 text-base flex items-center gap-2 mt-1">
            <span className={`w-2.5 h-2.5 rounded-full ${isAccepted ? 'bg-emerald-500' : isRejected ? 'bg-emerald-500' : 'bg-emerald-500 animate-ping'}`}></span>
            Negotiating with: {otherPartyName}
          </h3>
          <p className="text-xs text-emerald-500 flex items-center gap-1 mt-0.5">
            <MapPin size={12} className="text-emerald-400" /> {otherPartyLocation}
          </p>
        </div>
        
        {/* Deal Pricing Snapshot */}
        <div className="flex items-center gap-4 text-right">
          <div className="pr-4 border-r border-emerald-200 hidden sm:block">
            <div className="text-[10px] uppercase tracking-wider text-emerald-400 font-semibold">Farmer Asking</div>
            <div className="text-sm font-bold text-emerald-600">₹{negotiation.askingPrice}/{negotiation.unit}</div>
          </div>
          <div>
            ...
            <div className="text-2xl font-black text-emerald-600 tracking-tight">
              ...
              <span className="text-xs text-emerald-500 font-medium">/{negotiation.unit}</span>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-white space-y-4">
        ...
        <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-tranemerald-x-px md:before:mx-auto md:before:tranemerald-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-emerald-300 before:to-transparent pt-4 pb-4">
          {negotiation.history.map((msg, idx) => {
            ...
            const dotColor = isAcceptMsg ? 'bg-emerald-500' : isRejectMsg ? 'bg-emerald-500' : (msg.role === 'farmer' ? 'bg-emerald-600' : 'bg-emerald-600');

            return (
              <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                {/* Timeline dot */}
                <div className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-emerald-50 shrink-0 md:order-1 md:group-odd:-tranemerald-x-1/2 md:group-even:tranemerald-x-1/2 shadow-sm ${dotColor} text-white absolute left-0 md:left-1/2 md:-tranemerald-x-1/2 z-10`}>
                  <IconC size={16} />
                </div>
                
                {/* Card */}
                <div className={`w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-emerald-50 p-4 rounded-xl border ${isAcceptMsg ? 'border-emerald-200 shadow-emerald-100' : isRejectMsg ? 'border-emerald-200 shadow-emerald-100' : 'border-emerald-200'} shadow-sm ml-14 md:ml-0 hover:shadow-md transition-shadow relative`}>
                  {/* Small arrow pointing to center line on MD screens */}
                  <div className={`hidden md:block absolute top-5 w-3 h-3 bg-emerald-50 border-t border-r ${isAcceptMsg ? 'border-emerald-200' : isRejectMsg ? 'border-emerald-200' : 'border-emerald-200'} transform group-odd:rotate-45 group-odd:-left-1.5 group-even:-rotate-135 group-even:-right-1.5`}></div>
                  
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-emerald-900 text-sm">{msg.sender}</span>
                      <span className={`text-[9px] px-1.5 py-0.5 rounded font-extrabold uppercase tracking-wider ${
                        msg.role === 'farmer' ? 'bg-emerald-100 text-emerald-800' : 'bg-emerald-100 text-emerald-800'
                      }`}>
                        {msg.role}
                      </span>
                    </div>
                    <span className="text-[10px] font-semibold text-emerald-400">{msg.time}</span>
                  </div>
                  
                  <div className={`flex items-center justify-between gap-3 pb-3 mb-3 border-b ${isAcceptMsg ? 'border-emerald-100' : isRejectMsg ? 'border-emerald-100' : 'border-emerald-100'}`}>
                    <div className="flex flex-col">
                      <span className="text-[10px] uppercase tracking-wider text-emerald-500 font-bold mb-0.5">
                        {isInitial ? "Initial Listing" : isCounter ? "Counter-Bid" : isOffer ? "Offer Placed" : isAcceptMsg ? "Deal Finalized" : "Deal Declined"}
                      </span>
                      <span className={`font-black text-xl tracking-tight ${isAcceptMsg ? 'text-emerald-600' : isRejectMsg ? 'text-emerald-600' : 'text-emerald-900'}`}>
                        ₹{msg.price}<span className="text-sm font-semibold text-emerald-500 ml-0.5">/{negotiation.unit}</span>
                      </span>
                    </div>
                    <div className="text-right flex flex-col items-end">
                      <span className="text-[10px] uppercase tracking-wider text-emerald-500 font-bold mb-0.5">Total Value</span>
                      <span className={`font-black px-2.5 py-1 rounded-lg text-xs ${isAcceptMsg ? 'bg-emerald-50 text-emerald-700' : isRejectMsg ? 'bg-emerald-50 text-emerald-700' : 'bg-emerald-100 text-emerald-700'}`}>
                  ...
                  {msg.message && (
                    <div className={`text-xs font-medium leading-relaxed p-3 rounded-lg border ${isAcceptMsg ? 'bg-emerald-50/50 border-emerald-100 text-emerald-800' : isRejectMsg ? 'bg-emerald-50/50 border-emerald-100 text-emerald-800' : 'bg-emerald-50 border-emerald-100 text-emerald-600'}`}>
                      "{msg.message}"
                    </div>
                  )}
                </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-emerald-200 shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button 
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-emerald-900 hover:bg-black text-white font-bold py-3 px-4 rounded-xl transition-all shadow-sm cursor-pointer text-xs"
              >
                <Truck size={16} />
      ...
      {isRejected && (
        <div className="p-4 sm:p-5 bg-emerald-50 border-t border-emerald-200 z-10 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div>
          ...
          <button
            ...
            className="w-full sm:w-auto bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold px-5 py-2.5 rounded-xl transition-colors cursor-pointer shadow-sm"
          >
      ...
      {!isAccepted && !isRejected && isMyTurn && (
        <div className="bg-white border-t border-emerald-200 z-10 shadow-[0_-8px_20px_-10px_rgba(0,0,0,0.1)]">
          {/* Turn Prompt Header */}
          <div className="bg-emerald-50/80 px-4 py-2 border-b border-emerald-100 flex items-center justify-between text-xs">
            ...
            <button
              ...
              className="text-emerald-400 hover:text-emerald-600 text-[11px] font-semibold transition-colors cursor-pointer"
            >
              Decline Deal
            </button>
          </div>
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
            ...
            <div className="hidden md:flex md:col-span-1 items-center justify-center">
              <span className="text-xs font-black text-emerald-400 bg-emerald-100 px-2 py-1 rounded-full uppercase tracking-wider">
                OR
              </span>
            </div>

            {/* OPTION 2: GIVE ANOTHER OFFER (COUNTER-OFFER) */}
            <div className="md:col-span-7 bg-white p-3.5 rounded-2xl border border-emerald-200 shadow-sm flex flex-col justify-between">
              <div className="flex items-center justify-between mb-3">
                <span className="text-[10px] uppercase tracking-wider text-emerald-600 font-extrabold flex items-center gap-1.5">
                  <RefreshCw size={12} className="text-emerald-400" /> Option 2: Submit Counter-Bid
                </span>
                  ...
                  {isFarmer ? (
                    <>
                      ...
                      <button
                        ...
                        className="text-[10px] font-bold px-2 py-0.5 rounded border border-emerald-300 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors cursor-pointer"
                      >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                <div className="sm:col-span-4 relative">
                  <label className="block text-[9px] uppercase tracking-wider text-emerald-500 font-bold mb-1">Proposed Rate</label>
                  <div className="relative">
                    <span className="absolute left-3 top-2 text-emerald-500 font-bold text-sm">₹</span>
                    <input 
                      type="number" 
                      placeholder="Price"
                      value={inputPrice}
                      onChange={(e) => setInputPrice(e.target.value)}
                      className="w-full pl-8 pr-8 py-1.5 bg-emerald-50 border border-emerald-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 font-black text-emerald-900 transition-shadow"
                    />
                    <span className="absolute right-3 top-2 text-[10px] text-emerald-400 font-medium">/{negotiation.unit}</span>
                  </div>
                </div>

                <div className="sm:col-span-8 relative">
                  <label className="block text-[9px] uppercase tracking-wider text-emerald-500 font-bold mb-1">Formal Note (Optional)</label>
                  <div className="flex gap-2">
                    <input 
                      ...
                      className="w-full px-3 py-1.5 bg-emerald-50 border border-emerald-300 rounded-lg text-xs text-emerald-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-shadow"
                    />
                    <button 
                      onClick={handleSendCounter}
                      disabled={!inputPrice || Number(inputPrice) <= 0}
                      className="bg-emerald-900 hover:bg-black disabled:opacity-50 text-white font-bold rounded-lg px-4 py-1.5 text-xs flex items-center justify-center transition-colors cursor-pointer shrink-0 shadow-sm"
                    >
                      Submit
                    </button>
                  </div>
      ...
      {!isAccepted && !isRejected && !isMyTurn && (
        <div className="p-4 sm:p-5 bg-emerald-50/70 border-t border-emerald-200 z-10">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 rounded-2xl bg-emerald-100 border border-emerald-300 flex items-center justify-center text-emerald-700 shrink-0 shadow-2xs">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="bg-emerald-200/80 text-emerald-900 text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase">
                    Their Turn
                  </span>
                </div>
      ...
      {showTermsModal && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
            ...
            <div className="p-6 text-xs text-emerald-700 space-y-4">
              <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                <div className="text-emerald-900 font-bold mb-1">Agreement Summary:</div>
                <div className="flex justify-between text-emerald-700 font-medium">
                  <span>Produce:</span>
                  <span className="font-bold text-emerald-900">{negotiation.crop}</span>
                </div>
                <div className="flex justify-between text-emerald-700 font-medium mt-1">
                  <span>Quantity:</span>
                  <span className="font-bold text-emerald-900">{negotiation.quantity} {negotiation.unit}</span>
                </div>
                <div className="flex justify-between text-emerald-700 font-medium mt-1">
                  <span>Agreed Price:</span>
                  <span className="font-black text-emerald-700">₹{negotiation.currentOffer}/{negotiation.unit}</span>
                </div>
                <div className="flex justify-between text-emerald-700 font-bold border-t border-emerald-200 mt-2 pt-1 text-sm text-emerald-900">
              ...
              <ul className="list-disc pl-5 space-y-1.5 text-emerald-600">
              ...
              <label className="flex items-start gap-3 p-3 bg-emerald-50 border border-emerald-200 rounded-xl cursor-pointer mt-4 hover:bg-emerald-100/80 transition-colors">
                <input 
                  type="checkbox" 
                  checked={agreedToTerms}
                  onChange={(e) => setAgreedToTerms(e.target.checked)}
                  className="mt-0.5 w-4 h-4 text-emerald-600 border-emerald-300 rounded focus:ring-emerald-500 cursor-pointer" 
                />
                <span className="font-bold text-emerald-900 leading-tight">
                  I accept this offer of ₹{negotiation.currentOffer}/{negotiation.unit} and agree to legally binding terms.
                </span>
              </label>
            </div>

            <div className="p-4 bg-emerald-50 border-t border-emerald-100 flex justify-end gap-3">
              <button 
                onClick={() => { setShowTermsModal(false); setAgreedToTerms(false); }}
                className="px-4 py-2 font-semibold text-emerald-600 hover:text-emerald-900 cursor-pointer text-xs"
              >
                Cancel
              </button>
              <button 
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-sm w-full shadow-2xl p-5 animate-in fade-in zoom-in-95 space-y-4">
            ...
            <input 
              ...
              className="w-full px-3 py-2 border border-emerald-300 rounded-xl text-xs"
            />
            <div className="flex justify-end gap-2 pt-2">
              <button 
                onClick={() => setShowRejectModal(false)}
                className="px-3 py-2 text-xs font-semibold text-emerald-600 hover:text-emerald-900 cursor-pointer"
              >
                Back
              </button>
              <button 
      ...
      {showInvoiceModal && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl animate-in fade-in zoom-in-95 h-[80vh] flex flex-col overflow-hidden">
            {/* Modal Header */}
            <div className="p-3 bg-emerald-100 border-b border-emerald-200 flex items-center justify-between shrink-0">
              ...
              <button onClick={() => setShowInvoiceModal(false)} className="text-emerald-500 hover:text-emerald-800 cursor-pointer">
                <X size={20} />
              </button>
            </div>
            
            {/* PDF Content Area */}
            <div className="flex-1 overflow-y-auto bg-emerald-200 p-4 sm:p-8 flex justify-center">
              <div className="bg-white w-full max-w-md shadow-lg p-8 space-y-6 text-emerald-800 font-sans text-[11px] relative">
                ...
                <div className="flex justify-between items-start border-b-2 border-emerald-800 pb-4">
                  <div>
                    ...
                    <p className="text-emerald-500">Digital Smart Contract Receipt</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div>
                    <h3 className="font-bold text-emerald-400 uppercase tracking-wider mb-1 text-[9px]">Seller (Farmer)</h3>
                    <p className="font-bold text-sm">{negotiation.farmerName}</p>
                    <p className="text-emerald-600">{negotiation.farmerLocation}</p>
                  </div>
                  <div>
                    <h3 className="font-bold text-emerald-400 uppercase tracking-wider mb-1 text-[9px]">Buyer</h3>
                    <p className="font-bold text-sm">{negotiation.buyerName}</p>
                    <p className="text-emerald-600">{negotiation.buyerLocation}</p>
                  </div>
                </div>
                ...
                <table className="w-full mt-6">
                  ...
                  <tbody className="border-b border-emerald-200">
                    <tr>
                      <td className="py-3 px-2">
                        <p className="font-bold">{negotiation.crop}</p>
                        <p className="text-emerald-500 text-[9px]">Grade A, Inspected</p>
                      </td>
                ...
                {/* Footer Notes */}
                <div className="mt-8 pt-4 border-t border-emerald-200 text-[9px] text-emerald-500 space-y-1">
            ...
            {/* Modal Actions */}
            <div className="p-4 bg-white border-t border-emerald-200 flex justify-end gap-3 shrink-0">
              <button 
                onClick={() => {
